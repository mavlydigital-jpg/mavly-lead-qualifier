...
export function ScrapeMapsModal({
  open,
  onClose,
  onImported,
}: {
  ...
  const start = trpc.leads.scrapeMapsStart.useMutation();
  const importRun = trpc.leads.scrapeMapsImport.useMutation();
  const poll = trpc.leads.scrapeMapsPoll.useQuery(
    { runId: runId || "" },
  ...
  useEffect(() => {
    if (phase !== "running" || !poll.data || !runId) return;
    ...
    if (DONE.has(status)) {
      ...
      setPhase("importing");
      importRun
        .mutateAsync({ runId, max, nicheOnly, fileName: `Maps · ${search} · ${location}` })
        .then((res) => {
          toast.success(
            res.dropped
              ? `Imported ${res.imported} leads · dropped ${res.dropped} off-niche.`
              : `Imported ${res.imported} Google Maps leads.`,
          );
          reset();
      ...
      toast.error("Google Maps scrape failed. Try again.");
      setPhase("idle");
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [poll.data, phase, runId]);
      ...
      <div className="glass-card w-full max-w-md rounded-[28px] p-6">
        <div className="mb-4 flex items-center justify-between">
          <div className="flex items-center gap-2">
            <span className="grid h-9 w-9 place-items-center rounded-xl bg-[linear-gradient(135deg,#16A34A,#22C55E)] text-white shadow-[0_0_22px_rgba(34,197,94,0.35)]">
              <MapPin className="h-4 w-4" />
            </span>
            <h2 className="text-lg font-black tracking-tight">Scrape Google Maps</h2>
          </div>
          <button
        ...
        {busy ? (
          <div className="flex flex-col items-center gap-3 py-8 text-center">
            ...
            <p className="text-sm font-semibold">
              {phase === "running" ? "Scraping Google Maps…" : "Importing leads…"}
            </p>
            <p className="text-xs text-muted-foreground">
              Pulling listings from Google Maps. Keep this window open.
            </p>
          </div>
        ) : (
          <div className="flex flex-col gap-3">
              ...
              {["Foundation Repair", "Waterproofing", "Crawl Space Repair"].map((k) => (
                <button
                  ...
                  className={
                    ...
                    (search === k
                      ? "border-[#22C55E]/50 bg-[#16A34A]/15 text-[#9ff0b8]"
            ...
            <button
              onClick={run}
              className="mt-1 inline-flex h-11 w-full items-center justify-center gap-2 rounded-2xl bg-[linear-gradient(135deg,#16A34A,#22C55E)] text-sm font-bold text-white shadow-[0_16px_34px_rgba(34,197,94,0.28)] transition hover:brightness-110 active:scale-[0.98]"
            >
              <Search className="h-4 w-4" /> Run scrape
            </button>
            <p className="text-[11px] text-muted-foreground">
              Pulls Google Maps listings (name, phone, website, city, rating) into a new session. Phone numbers only — built for cold calling & SMS.
            </p>
          </div>
        )}
      </div>
...
const inputCls =
  "h-11 w-full rounded-2xl border border-white/[0.1] bg-[#0A142D]/70 px-4 text-sm text-[#EAF0FF] outline-none placeholder:text-muted-foreground/70 focus:border-[#22C55E]/60 focus:ring-2 focus:ring-[#16A34A]/20";
